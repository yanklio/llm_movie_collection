...
class MovieAPITool:
    ...
    def search_by_imdb_id(self, imdb_id: str) -> MovieInfo:
        ...
        response = requests.get(f"{self.BASE_URL}/find/{imdb_id}", params=params, timeout=10)
        response.raise_for_status()
        data = response.json()
    ...
    def search_person(self, person_name: str) -> list[dict]:
        ...
        response = requests.get(f"{self.BASE_URL}/search/person", params=params, timeout=10)
        response.raise_for_status()
        data = response.json()
    ...
    def _get_movie_details(self, tmdb_id: int) -> MovieInfo:
        ...
        response = requests.get(f"{self.BASE_URL}/movie/{tmdb_id}", params=params, timeout=10)
        response.raise_for_status()
        data = response.json()
    ...
    def search_by_keyword(self, keyword: str) -> list[dict]:
        ...
        Args:
            keyword: Search keyword (e.g., "sci-fi", "thriller", "bad rating")

        Returns:
            List of movies matching the keyword
        """
        # For generic keywords, use movie search
        params = {
            "api_key": self.api_key,
            "query": keyword,
        }
    ...
    def search_by_person(self, person_name: str) -> list[dict]:
        """
        Wrapper method for search_person to match MovieCollector expectations.
        ...
        Returns:
            List of movies the person acted in or directed
        """
        return self.search_person(person_name)
